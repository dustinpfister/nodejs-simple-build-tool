...
api.writeDist = (dist) => {
    ...
    return mkdirp.promise(dist.dir_target)
    // write the dev version
    .then(() => {
        return writeFile( path.join( dist.dir_target, 'file.js' ), dist.sourceCode, 'utf8' );
    })
    // write the min version
    .then(()=>{
        return writeFile( path.join( dist.dir_target, 'file.min.js' ), dist.minCode, 'utf8' );
    });
};
