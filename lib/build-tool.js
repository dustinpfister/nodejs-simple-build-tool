...
api.writeDist = (dist) => {
    ...
    .then(() => {
        return writeFile( path.join( dir_target, dist.fileName + '.js' ), dist.sourceCode, 'utf8' );
    })
    // write the min version
    .then(()=>{
        return writeFile( path.join( dir_target, dist.fileName + '.min.js' ), dist.minCode, 'utf8' );
    }).
    then(()=>{
        return dist;
    });
