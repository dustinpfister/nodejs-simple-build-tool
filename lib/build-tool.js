...
// return a promise that will contain a source code string with the given opt object 
// created directly or with buildTool.readConf
api.createSource = (opt) => {
    ...
    // to any path value.
    cdOpt.dir_root = opt.dir_root;

    // sourceCode starts out as an empty string
    cdOpt.sourceCode = '';
    ...
    return Promise.all(opt.sourceFiles.map((uri_source)=>{
        return readFile( path.join(cdOpt.dir_root, uri_source), 'utf8' );
    }))
    .then((codeArray) => {
        return {
            error: null,
...
/*
    opt.sourceFiles.forEach((uri_source)=>{
